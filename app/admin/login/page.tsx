...
export default function AdminLoginPage() {
    ...
    const handleLogin = async (e: React.FormEvent) => {
        ...
        if (!passcode.trim()) { toast.error('Passcode required'); return; }
        setLoading(true);
        const { error } = await adminLoginEmail(email.trim(), passcode.trim());
        if (error) {
            toast.error(error);
            setLoading(false);
            return;
            ...
            <div className="w-full max-w-sm">
                ...
                <div className="bg-card border border-bdr rounded-2xl p-6">
                    <form onSubmit={handleLogin} className="space-y-4">
                        ...
                        <div>
                            <label className="block text-[10px] font-bold tracking-[0.12em] uppercase text-t3 mb-2">Passcode</label>
                            <input className="w-full bg-deep border border-bdr rounded-xl px-4 py-3 text-sm outline-none placeholder:text-t3 focus:border-lime/40 transition-colors"
                                type="password" placeholder="••••" value={passcode} onChange={(e) => setPasscode(e.target.value)} />
                        </div>
